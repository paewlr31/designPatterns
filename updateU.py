...
PASSWORD_LENGTH = 6
TASK_TIMEOUT = 30  # sekundy – po tym paczka wraca do puli, jeśli node nie zgłasza aktywności
...
class DistributedBruteForcer:
    def __init__(self):
        ...
        # Po starcie: poproś o SYNC i wyślij swoje dane
        time.sleep(1)
                ...
                if text.startswith("HELLO:"):
                    ...
                    if ip != self.ip:
                        ...
                        if was_new:
                            ...
                            self._log_status()
                            # Poproś o SYNC od nowego peer'a
            ...
            try:
                ...
                if msg.startswith("TASK_START:"):
                    ...
                    if len(self.done_batches) > old_count:
                        print(f"[SYNC] Otrzymano {len(done)} zrobionych paczek od {peer_ip}")
                        self._log_status()
