...
class DistributedBruteForcer:
    def __init__(self, initial_password=None):
        ...
        self.target_password = initial_password
    ...
    def _establish_target_and_network(self):
        ...
        # === ROZPOCZYNAMY PRACĘ PO USTALENIU CELU ===
        self.sync_ready.set()
        self._log_status()
        ...
        while not self.global_stop:
            try:
                ...
                if msg.startswith("PING:"):
                    ...
                    with self.lock:
                        old_done = len(self.done_batches)
                        self.done_batches |= done

                        # Ustawiamy cel, jeśli jeszcze nie znamy
                        if not self.target_hash and hash_part and length_part:
                            self.target_hash = hash_part
                            self.password_length = int(length_part)
                            print(f"[SYNC] Otrzymano cel: SHA1 = {hash_part[:8]}..., długość = {self.password_length}")
                            self.sync_ready.set()
                            # Uruchamiamy pracę od razu, jeśli jeszcze nie ruszyła
                            if not hasattr(self, '_work_started') or not self._work_started:
                                self._work_started = True
                                threading.Thread(target=self._work_loop, daemon=True).start()

                    if len(self.done_batches) > old_done:
    ...
    def _broadcast_sync(self):
        with self.lock:
            ...
            hash_part = self.target_hash or ""
            length_part = str(self.password_length) if self.password_length is not None else ""
            msg = f"SYNC:{csv}:{hash_part}:{length_part}".encode()
        try:
            self.mcast_sock.sendto(msg, (MULTICAST_GROUP, MULTICAST_PORT))
        except:
            pass
