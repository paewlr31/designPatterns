...
def generate_random_hash():
    pwd = ''.join(random.choice(CHARSET) for _ in range(PASSWORD_LENGTH))
...
class DistributedBruteForcer:
    def __init__(self):
        ...
        self.target_hash = None  # ← TUTAJ PRZECHOWUJEMY HASH
        self.master_node = None  # ← IP mastera
    ...
    def _establish_target(self):
        if self.target_hash is not None:
            print(f"[INFO] Cel: {self.target_hash[:8]}...")
            return

        if not self.peers:
            self.is_master = True
            self.master_node = self.ip
            self.target_hash = generate_random_hash()
            print(f"[MASTER] Wylosowałem cel: {self.target_hash}")
            self._broadcast_target()
        else:
            print("[SLAVE] Czekam na cel...")
            self._wait_for_target()

    def _wait_for_target(self):
        if not self.sync_ready.wait(15):
            print("[ERROR] Brak celu – kończę.")
            self.global_stop = True

    def _broadcast_target(self):
        msg = f"TARGET:{self.target_hash}".encode()
        try:
        ...
        while not self.global_stop:
            try:
                ...
                if msg.startswith("PING:"):
                    ...
                    parts = msg.split("|")
                    done_csv = parts[0][5:] if parts[0].startswith("SYNC:") else ""
                    target_part = parts[1] if len(parts) > 1 else ""
                    done = {int(x) for x in done_csv.split(",") if x}
                    with self.lock:
                    ...
                    if target_part.startswith("TARGET:"):
                        _, h = target_part.split(":", 1)
                        if self.target_hash is None or self.target_hash != h:
                            self.target_hash = h
                            self.master_node = ip
                            print(f"[TARGET] Nowy cel od {ip}: {h}")
                            self.sync_ready.set()
    ...
    def _broadcast_sync(self):
        if not self.target_hash: return
        with self.lock:
            csv = ",".join(map(str, sorted(self.done_batches)))
        msg = f"SYNC:{csv}|TARGET:{self.target_hash}".encode()
        try:
            self.mcast_sock.sendto(msg, (MULTICAST_GROUP, MULTICAST_PORT))
        except:
            pass

    def _send_unicast_sync_to(self, ip):
        if not self.target_hash: return
        with self.lock:
            csv = ",".join(map(str, sorted(self.done_batches)))
        msg = f"SYNC:{csv}|TARGET:{self.target_hash}".encode()
    ...
    def _send_sync_request(self):
        for ip in list(self.peers.keys()):
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                s.sendto(b"SYNC_REQ:", (ip, TASK_PORT))
                s.close()
            except:
                pass
    ...
    def _work_loop(self):
        while not self.global_stop and self.target_hash:
